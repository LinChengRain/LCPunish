...
static OrderDBManager* orderDBManager;

@implementation OrderDBManager
...
//        NSString *tmpDir = [NSTemporaryDirectory() stringByAppendingPathComponent:SQLITE_NAME];
//         NSLog(@"%@",tmpDir);
        // 1.创建数据库队列
        self.queue = [FMDatabaseQueue databaseQueueWithPath:filename];
        NSLog(@"数据库初始化-----%@",filename);
    }
    return self;
}
/**
...
+(instancetype)intance{
    if(orderDBManager == nil){
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            orderDBManager = [[OrderDBManager alloc] init];
...
-(BOOL)updateOneWithTableName:(NSString *)name valueDict:(NSDictionary *)valueDict where:(NSArray *)where index:(NSInteger)index{
