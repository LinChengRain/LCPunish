...
#import "LCCustomListCell.h"
@interface LCOverContainerView : UIView
...
@implementation LCOverContainerView
...
@interface LCCustomListView () <UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong) LCOverContainerView *containerView; // black backgroud container
...
- (LCOverContainerView *)containerView
{
    if (nil == _containerView) {
        _containerView = [[LCOverContainerView alloc]init];
...
{
    static NSString *identifier = @"LCCustomListCell";
//    LCCustomListCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (nil == cell) {
//        cell = [[[NSBundle mainBundle] loadNibNamed:@"LCCustomListCell" owner:nil options:nil]lastObject];
        
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:identifier];
        
    }
    // 获取数据
    NSDictionary*dic = self.titleMenus[indexPath.row];
    
//    cell.username.text = dic[@"username"];
//
//    cell.department.text = dic[@"department"];
